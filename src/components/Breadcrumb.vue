...
<div class="breadcrumb flex items-center w-full">
    <div class="flex items-center" v-for="(dirpath, index) in breadcrumb" :key="dirpath.path">
      ...
      <ChevronRight v-if="index != breadcrumb.length - 1" class="stroke-current text-gray-400 w-4 h-4"/>
    </div>
</div>
</template>
    ...
    computed:{
        ...
        breadcrumb() {
            ...
            for (let i = 0; i < splitter.length; i++) {
                if (!recurnav.includes(topfolder))
                    break;
                if (i > 0)
                    recurnav = path.dirname(recurnav)
                console.log(recurnav)
                navigo.push({
                    path: recurnav,
                    name: path.basename(recurnav)
                })
            }
            console.log(navigo.filter(function (navobj) {
                return navobj.path.includes(topfolder);
            }))
        
            return navigo.filter(function (navobj) {
                return navobj.path.includes(topfolder);
            }).reverse()
